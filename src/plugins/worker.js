///=========================================================================
/// This module provides the worker feature for baguette-script
/// Plugin from _hyperscript
///=========================================================================
	...
	return (baguetteScript) => {
		...
		var workerFunc = function (self) {
			self.onmessage = function (e) {
				switch (e.data.type) {
					case "init":
						self.importScripts(e.data.baguetteScript);
						self.importScripts.apply(self, e.data.extraScripts);
						const baguetteScript = self['baguetteScript']
						var tokens = new baguetteScript.internals.Tokens(e.data.tokens, [], e.data.source);
						var _baguetteScript = baguetteScript.internals.parser.parseElement("baguette-script", tokens);
						_baguetteScript.apply(self, self);
						postMessage({ type: "didInit" });
						break;
					case "call":
						try {
							var result = self['baguetteScript'].internals.runtime
								.getBaguetteScriptFeatures(self)[e.data.function]
		...
		baguetteScript.addFeature("worker", function (parser, runtime, tokens) {
			if (tokens.matchToken("worker")) {
				...
				worker.postMessage({
					type: "init",
					baguetteScript: runtime.baguetteScriptUrl,
